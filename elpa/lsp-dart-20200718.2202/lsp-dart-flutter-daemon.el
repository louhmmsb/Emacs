...
(defvar lsp-dart-flutter-daemon--device-enable-timer nil)

(defun lsp-dart-flutter-daemon--device-enable ()
  "Check if process already started then enable device pooling."
  (message "Checking for process...")
  (when (get-buffer-create lsp-dart-flutter-daemon-buffer-name)
    (when lsp-dart-flutter-daemon--device-enable-timer
      (cancel-timer lsp-dart-flutter-daemon--device-enable-timer)
      (setq lsp-dart-flutter-daemon--device-enable-timer nil))
    (message "Process OK, sending device.enable")
...
(defun lsp-dart-flutter-daemon-start ()
  ...
  (unless (lsp-dart-flutter-daemon--running-p)
    (let ((buffer (get-buffer-create lsp-dart-flutter-daemon-buffer-name)))
      ...
      (setq lsp-dart-flutter-daemon--device-enable-timer
            (run-with-idle-timer 1 t #'lsp-dart-flutter-daemon--device-enable)))))
